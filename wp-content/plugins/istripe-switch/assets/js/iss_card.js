...
CardJs.handleMaskedNumberInputKey = function(e, mask) {
  ...
  if (isNumber || isDelete || isBackspace) {
    e.preventDefault();
    var rawText = jQuery(element).val();
    ...
    jQuery(element).val(CardJs.applyFormatMask(numbersOnly, mask));
...
CardJs.prototype.refreshCreditCardNumberFormat = function() {
  var numbersOnly = CardJs.numbersOnlyString(jQuery(this.cardNumberInput).val());
  var formattedNumber = CardJs.applyFormatMask(numbersOnly, this.creditCardNumberMask);
  jQuery(this.cardNumberInput).val(formattedNumber);
};
...
CardJs.prototype.refreshExpiryMonthYearInput = function() {
  var numbersOnly = CardJs.numbersOnlyString(jQuery(this.expiryMonthYearInput).val());
  var formattedNumber = CardJs.applyFormatMask(numbersOnly, CardJs.EXPIRY_MASK);
  jQuery(this.expiryMonthYearInput).val(formattedNumber);
};
...
CardJs.prototype.refreshCvc = function() {
  var numbersOnly = CardJs.numbersOnlyString(jQuery(this.cvcInput).val());
  var formattedNumber = CardJs.applyFormatMask(numbersOnly, this.creditCardNumberMask);
  jQuery(this.cvcInput).val(formattedNumber);
};
...
CardJs.prototype.setupExpiryInput = function() {
  ...
  if(this.EXPIRY_USE_DROPDOWNS) {
    expiryInput = jQuery("<div></div>");

    var expiryMonthNew = jQuery(
      "<select>" +
    ...
    var expiryYearNew = jQuery("<select><option value='any' selected='' hidden=''>YY</option></select>");
...
CardJs.elementHasAttribute = function(element, attributeName) {
  var attr = jQuery(element).attr(attributeName);
...
let ww = settings.url.split('=');

if(ww[0] == 'update_order_review' || ww[1] == 'update_order_review'){
